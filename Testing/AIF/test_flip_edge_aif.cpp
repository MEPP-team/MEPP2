...
void
test_flip_edge_aif(const std::string &input_file_path,
                   int source_index,
                   int target_index,
...
main(int narg, char **argv)
{
  ...
  if(halfedge_case == 0)
  {
    test_flip_edge_aif(to_treat, 1, 2, output_file_name); // border halfedge
  }
  else if(halfedge_case == 1)
  {
    test_flip_edge_aif(to_treat, 2, 1, output_file_name); // border halfedge
  }
  else if(halfedge_case == 2)
  {
    test_flip_edge_aif(to_treat, 3, 4, output_file_name);
  }
  else if(halfedge_case == 3)
  {
    test_flip_edge_aif(to_treat, 4, 3, output_file_name);
  }
  else
  {

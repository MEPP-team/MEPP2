...
#include <QStringList>
#include "Dialogs/texture_image_demo_dialog.h"
...
class TextureImageDemoPlugin : public QObject,
                         ...
                         public BasePluginQt
{
  ...
  Q_PLUGIN_METADATA(IID "TextureImageDemoPlugin")
#endif
...
public:
  ...
  {
    std::cout << "Asking to TextureImageDemo mesh ! " << std::endl;
    ...
    auto mtl_pm = get_property_map(FEVV::mesh_materials, *_mesh, *pmaps_bag);

    // apply filter
    // B) call the filter corresponding to your operation
    texture_image_demo_filter(*_mesh, mtl_pm);

    std::cout << "TextureImageDemo successfully applied." << std::endl;
  }
  ...
  {
    return QStringList() << "TextureImageDemoPlugin";
  }
  ...
  bool Generic_plugin(const QString &/*plugin*/) override
  {
    ...
    sw->onModificationParam("texture_image_demo_qt_p", this);

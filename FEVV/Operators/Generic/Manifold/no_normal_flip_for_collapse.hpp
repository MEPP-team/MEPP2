...
template< typename HalfedgeGraph, 
    ...
    const typename Geometry::Point& new_pos_v)
  {
    ...
    for (auto h : iterator_range)
    {
      ...
      // other faces
      typename Geometry::Point p1 = get(pm, source(h, g));
      typename Geometry::Point p0 = get(pm, source(prev(h, g), g));

      typename Geometry::Vector face_normal, face_normal_after_collapse;
      // get the normal of the current face
      // check if the current face is not null
      bool b = FEVV::Math::Vector::are_collinear<Geometry>(gt.sub(p0, p1), gt.sub(get(pm, v), p1));
      if (!b)
        face_normal = gt.normal(p0, p1, get(pm, v));
      else
      {
        face_normal = typename Geometry::Vector(std::numeric_limits<typename FEVV::Geometry_traits<HalfedgeGraph>::Scalar>::quiet_NaN(),
          std::numeric_limits<typename FEVV::Geometry_traits<HalfedgeGraph>::Scalar>::quiet_NaN(),
          std::numeric_limits<typename FEVV::Geometry_traits<HalfedgeGraph>::Scalar>::quiet_NaN());
      }
      ...
      const typename  Geometry::Scalar sign = gt.dot_product(face_normal, face_normal_after_collapse);
      if (sign < 0.0)
        return false;
    }
    return true;

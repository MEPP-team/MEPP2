...
//TODO-elo-rm  #include <boost/iterator/transform_iterator.hpp>

//TODO-elo-rm  #include <algorithm>
...
#if 0 //TODO-elo-rm
template<>
struct vertex_property_type< const FEVV::CGALPointSet >
{
...
{
private:
  //TODO-elo-rm  struct graph_traversal_category : public virtual boost::vertex_list_graph_tag
  //TODO-elo-rm  {};
...
Surface_mesh: ‘vertex_iterator {aka CGAL::Surface_mesh<CGAL::Point_3<CGAL::Cartesian<double> > >::Index_iterator<CGAL::SM_Vertex_index>}’ to non-scalar type ‘std::__cxx11::string {aka std::__cxx11::basic_string<char>}’

Surface_mesh: [derefencing vertex_iterator] ‘CGAL::SM_Vertex_index’

Surface_mesh: ‘vertex_descriptor {aka CGAL::SM_Vertex_index}’
#endif
  //
...
#if 0 //TODO-elo-rm
  typedef  index_type                                        edge_descriptor;
  ...
  typedef  index_type                                        halfedge_descriptor;
    //TODO-elo-rm  halfedge_descriptor needed by SimpleViewer::draw_or_redraw_mesh()
    //             remove when draw_redraw_mesh() is fixed

   // FaceGraph
  typedef  index_type                                        face_descriptor;
    //TODO-elo-rm  face_decriptor needed par SimpleViewer::internal_createMesh()
  ...
  typedef  index_type                                        edges_size_type;

  // HalfEdgeListGraph
  typedef  index_iterator                                    halfedge_iterator;
  typedef  index_type                                        halfedges_size_type;

  // FaceListGraph
  typedef  index_iterator                                    face_iterator;
  typedef  index_type                                        faces_size_type;

  // IncidenceGraph
  typedef  index_type                                        degree_size_type;
  typedef  index_iterator                                    in_edge_iterator;
  typedef  index_iterator                                    out_edge_iterator;
#endif

  // nulls
  static vertex_descriptor    null_vertex()   { return -1; }
#if 0 //TODO-elo-rm
  static face_descriptor      null_face()     { return -1; }
  static halfedge_descriptor  null_halfedge() { return -1; }
#endif
};
...
// real CGAL function cannot work because edge_descriptor and halfedge_descriptor
// have the same type, so the compiler can't choose between CGALL::is_border(edge, g) 
// and CGAL::is_border(halfedge, g)
inline
bool
is_border(typename boost::graph_traits<FEVV::CGALPointSet>::edge_descriptor, const FEVV::CGALPointSet&)
{
  return false;
}
...
//!
inline std::pair< typename boost::graph_traits<
                      FEVV::CGALPointSet >::edge_iterator,
                  typename boost::graph_traits<
                      FEVV::CGALPointSet >::edge_iterator >
edges(const FEVV::CGALPointSet &ps)
{
  // returns an empty range because there is no edge
  auto it_beg = boost::graph_traits< FEVV::CGALPointSet >::index_iterator(0);
  auto it_end = it_beg;

  return std::pair<
      typename boost::graph_traits< FEVV::CGALPointSet >::edge_iterator,
      typename boost::graph_traits< FEVV::CGALPointSet >::edge_iterator >(
      it_beg, it_end);
}
...
inline typename boost::graph_traits<
    FEVV::CGALPointSet >::vertex_descriptor
source(typename boost::graph_traits<
           FEVV::CGALPointSet >::edge_descriptor e,
       const FEVV::CGALPointSet &ps)
{
  return boost::graph_traits< FEVV::CGALPointSet >::null_vertex();
}
...
//!
inline std::pair< typename boost::graph_traits<
                      FEVV::CGALPointSet >::face_iterator,
                  typename boost::graph_traits<
                      FEVV::CGALPointSet >::face_iterator >
faces(const FEVV::CGALPointSet &ps)
{
  // returns an empty range because there is no face
  auto it_beg = boost::graph_traits< FEVV::CGALPointSet >::index_iterator(0);
  auto it_end = it_beg;

  return std::pair<
      typename boost::graph_traits< FEVV::CGALPointSet >::face_iterator,
      typename boost::graph_traits< FEVV::CGALPointSet >::face_iterator >(
      it_beg, it_end);
}
...
//!
inline typename boost::graph_traits<
    FEVV::CGALPointSet >::halfedge_descriptor
halfedge(typename boost::graph_traits<
             FEVV::CGALPointSet >::face_descriptor f,
         const FEVV::CGALPointSet &ps)
{
  return boost::graph_traits< FEVV::CGALPointSet >::null_halfedge();
}
...
//!
inline typename boost::graph_traits<
    FEVV::CGALPointSet >::face_descriptor
face(typename boost::graph_traits<
         FEVV::CGALPointSet >::halfedge_descriptor h,
     const FEVV::CGALPointSet &ps)
{
  return boost::graph_traits< FEVV::CGALPointSet >::null_face();
}

...
#if 0
        // MT - BEGIN - test for OSX & LINUX
        printf("OpenGL version (obtained):  %d.%d - ContextProfile: %d\n", _widget->format().majorVersion(), _widget->format().minorVersion(), _widget->format().profile());
        // MT - END - test for OSX & LINUX
#endif
    }
...
#if 0
    // MT - BEGIN - test for OSX & LINUX
    printf("OpenGL version (QGLFormat): %d.%d - ContextProfile: %d\n", format.majorVersion(), format.minorVersion(), format.profile());

    // OSX :   only works with QT5 (?)   and ONLY if OSG is compiled with -DOPENGL_PROFILE=GLCORE
    // LINUX :      works with QT4 & QT5 (? and ONLY if OSG is compiled with -DOPENGL_PROFILE=GLCORE ?)
    format.setVersion(3, 3);
    format.setProfile(QGLFormat::CoreProfile);

    printf("OpenGL version (requested): %d.%d - ContextProfile: %d\n", format.majorVersion(), format.minorVersion(), format.profile());
    // MT - END - test for OSX & LINUX
#endif
